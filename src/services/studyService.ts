...
export class StudyService {
  ...
  // SEARCH
  static async searchCourses(userId: string, query: string) {
    const { data, error } = await supabase
      .from('courses')
      .select('id, name, department_id, departments:department_id(id, name)')
      .eq('user_id', userId)
      .ilike('name', `%${query}%`);
    if (error) throw error;
    return data as any[];
  ...
  static async searchTopics(userId: string, query: string) {
    const { data, error } = await supabase
      .from('topics')
      .select(`
        id,
